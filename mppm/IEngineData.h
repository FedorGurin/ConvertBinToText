...
/*! \brief  Абстрактный класс для скрытия
 * конкретных реализаций "движков" управления данными
 *
 */
...
public:
    enum TYPE_REQUEST
    {
        E_REQ_SYNCH_ENGINE, /*синхронная обработка запроса*/
        E_REQ_ASYNCH_ENGINE,/*асинхронная обработка запроса*/
        E_REQ_CYCLIC_ENGINE /*циклический запрос*/
    };
    IEngineData(QObject* parent = nullptr):QThread(parent){}
    //! задать данные
    virtual TRequestEvent setValue(TCommonRequest request, TYPE_REQUEST type=E_REQ_ASYNCH_ENGINE)
    {
   ...
   /* virtual uint specialCommand(QStringList str1,QStringList str2,TYPE_REQUEST type=ASYNCH_ENGINE)
    ...
    //! получить данные
    virtual TRequestEvent getValue(TCommonRequest request,TYPE_REQUEST type=E_REQ_ASYNCH_ENGINE)
    {
        TRequestEvent answer0;
        Q_UNUSED(request);
        Q_UNUSED(type);
    ...
    virtual TRequestEvent command(TCommonRequest request, TYPE_REQUEST type = E_REQ_ASYNCH_ENGINE)
    {
        TRequestEvent answer0;
        Q_UNUSED(request);
        Q_UNUSED(type);
...
class CommonEngineData : public IEngineData
{
    Q_OBJECT
public:
    explicit CommonEngineData(QObject *parent = nullptr);
    ...
    virtual TRequestEvent command(TCommonRequest request,TYPE_REQUEST type = E_REQ_ASYNCH_ENGINE)
    {
        ...
        for(auto i : engines)
        {
            TRequestEvent answer0 = i->command(request,type);
